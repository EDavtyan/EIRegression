...
def execute(save_dir, n_buckets=3, i=None, bucketing_method="quantile"):
    # Load dataframe
    data = pd.read_csv("/Users/eddavtyan/Documents/XAI/Projects/EIRegression/examples/datasets/movies.csv")
    target = "gross"
    ...
    # Identifying numerical and categorical columns (excluding 'gross' from numerical)
    # Define columns for numerical transformations
    numerical_cols = ['num_critic_for_reviews', 'duration', 'budget',
                        'num_voted_users',
    ...
    numerical_cols_indices = [data.columns.get_loc(col) for col in numerical_cols if col in data.columns]

    # Preprocessing for numerical data: fill missing values with median
    numerical_transformer = SimpleImputer(strategy='median')

    # Preprocessing for categorical data: fill missing values with 'Unknown' and apply one-hot encoding
    ...
    preprocessor = ColumnTransformer(
        transformers=[
            ('num', numerical_transformer, numerical_cols_indices),
        ],
    ...
    regressor_optimizer = ModelOptimizer(search_method="random", n_iter=5)
...
def run_multiple_executions(save_dir, num_buckets, num_iterations):
    ...
    for n_buckets in range(1, num_buckets + 1):
        ...
        for iteration in range(1, num_iterations + 1):
            ...
            results = execute(save_dir=save_dir, n_buckets=num_buckets, i=iteration)
...
if __name__ == '__main__':
    save_dir = "/Users/eddavtyan/Documents/XAI/Projects/EIRegression/examples/results"  # Change this to your actual save directory
